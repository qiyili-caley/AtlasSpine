...
for label_file in os.listdir(label_dir):
    ...
    mask_color = cv2.cvtColor(mask, cv2.COLOR_GRAY2BGR)  # 用于画彩色箭头
    ...
    for line in lines:
        ...
        # ------ 画方向箭头 ------
        # 箭头长度建议为椎骨宽度一半
        arrow_len = max(w, h) * 0.5
        # 注意OpenCV的angle是角度，且0度朝右，正方向逆时针
        # 算终点坐标
        ...
        # 用绿色画箭头，线宽2
        cv2.arrowedLine(mask_color, pt1, pt2, (0,255,0), 2, tipLength=0.25)
        # 可选：标记椎骨id
        cv2.putText(mask_color, str(id), pt1, cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0,0,255), 2)

    # 输出mask+箭头图片
    out_path = os.path.join(out_dir, f"{img_name}_mask_dir.png")
    cv2.imwrite(out_path, mask_color)
    print(f"[OK] 已保存带方向箭头的mask：{repr(out_path)}")